...
<Update label="March 3, 2026" tags={["New releases", "Improvements"]} rss={{ title: "CIE funding workflows, handshake test path updates, and KmtPillarsVault" }}>

  ## CIE funding and workflow integration

  Added integration support for CIE funding orchestration and workflow execution in the EmtellRP Network Chain.

  ## Handshake test execution path update

  Updated the handshake testing flow to use the new execution path for more reliable CI runs and clearer workflow isolation.

  ## KmtPillarsVault contract and tests

  Implemented the `KmtPillarsVault` contract and added test coverage for vault initialization, funding paths, and workflow-linked contract behavior.
